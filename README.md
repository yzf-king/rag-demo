...
完整实验记录见 [`docs/evaluation.md`](docs/evaluation.md)。**一共做了两轮**：第一轮的结论逼我重写了切分器，第二轮才拿到真正有意义的对比。
...
### 第二轮（切分器改好后重跑）：参数真的生效了，而且小块胜出

切分器改了四处：标题跟随正文、段落贪心打包、长段按句切分、过短尾块合并（`RagService.split`）。

- **参数生效验证**：同一份扩展语料，cs=150 → 17 块、cs=300 → 7 块、cs=600 → 4 块（第一轮是"三档都切出 34 块"）；不足 40 字的块降到 0，标题块污染消失。
- **Q1–Q8 正确率**：150 组 8/8、300 组 8/8、600 组 7/8。三组每问的引用上下文开销差 4.6 倍（均 292 / 758 / 1335 字）。
- **大块不是"更保险"**：600 组一次把 3/4 份文档塞进 prompt，出现关键句被淹没导致漏答（Q5），以及拒答措辞自相矛盾（Q7）。
- **结论**：chunk-size 应对齐**文档的自然结构单位**（这份语料一节 ≈ 150 字），而不是拍个大数字"多给点上下文"——给多了反而稀释注意力，还按字数计费。
...
1. ✅ **切分器改进（已完成）**：标题与正文合并、段落贪心打包、句子级切分、过短尾块合并——参数因此真正生效，见 [docs/evaluation.md](docs/evaluation.md) 第二轮
2. **相似度阈值**：低于阈值直接拒答，减少"硬凑"引用
3. **Rerank**：召回 top-20 后接 bge-reranker 精排
4. **评估自动化**：接 RAGAS / LLM-as-judge，把人工对照预期升级为可回归的指标
5. **多跳问题**：设计"需要两节信息"的题，验证"小块 + 小 k"会不会漏（150 组 17 块、k=3 只覆盖 3 节，这是当前最可疑的短板）
